...
Set-Location -Path "C:\users\Neil-Insentra\OneDrive - Insentra Pty Ltd\Project Work\Cerberus - IPM\Azure DR Project 1\Build Scripts" # Optionally set the csv path
$VMs = import-csv .\DC7.csv # If using set-path option, no path is needed. Alternatively, the entire path can be defined here.
#endregion
...
Foreach ($v in $VMs) {
    ...
    if ($OS -eq "Windows") {
        ...
        $VirtualMachine = Set-AzVMOperatingSystem -VM $VirtualMachine -Windows -ComputerName $VMName -Credential $VMCredential -ProvisionVMAgent -EnableAutoUpdate:$false -PatchMode Manual
        ...
        if ($Disk0) {
            $DataDisk0Name = $VMName + '_dataDisk0'
            if ($VMZone) {
                $DataDisk0Config = New-AzDiskConfig -SkuName $Disk0SKU -Location $VMRegion -CreateOption Empty -DiskSizeGB $Disk0GB -Zone $VMZone
            }
            else {
                $DataDisk0Config = New-AzDiskConfig -SkuName $Disk0SKU -Location $VMRegion -CreateOption Empty -DiskSizeGB $Disk0GB
            }
            $DataDisk0 = New-AzDisk -DiskName $DataDisk0Name -Disk $DataDisk0Config -ResourceGroupName $VMResourceGroupName
            Add-AzVMDataDisk -VM $VirtualMachine -Name $DataDisk0Name -CreateOption Attach -ManagedDiskId $DataDisk0.Id -Lun 1 -Caching $Disk0Cache 
        }
        if ($Disk1) {
